...
function ProfileEditContent({
  ...
  return (
    <>
      <div className={`flex shrink-0 items-start justify-between gap-3 ${centered ? "text-center" : ""}`}>
        <h2
          id="profile-edit-modal-title"
          className={`text-lg font-semibold tracking-tight text-foreground ${centered ? "w-full" : ""}`}
        >
          Данные профиля
        </h2>
        {showHeaderClose ? (
      ...
      <p
        className={`text-sm leading-relaxed text-muted-foreground ${centered ? "mt-1 mb-4 text-center" : "mt-1 mb-4"}`}
      >
        Измените имя, фамилию и дату рождения
      </p>

      <div
        className={`min-h-0 space-y-3.5 overflow-y-auto overscroll-contain pr-0.5 pb-1 ${
          centered ? "max-h-[min(52vh,420px)]" : "max-h-[min(50dvh,380px)]"
        }`}
      >
      ...
      <div className={`mt-5 flex shrink-0 gap-3 ${centered ? "" : "border-t border-border/50 bg-card pt-5"}`}>
        <button
          type="button"
          onClick={() => !saving && onClose()}
          disabled={saving}
  ...
  if (isDesktop) {
    return (
      ...
      >
        ...
        />
        <div
          className={`${centerModalPanelClass} relative max-w-md ${centerModalPanelMotionClass(isVisible, isExiting)}`}
          onClick={(e) => e.stopPropagation()}
        >
          <ProfileEditContent {...contentProps} centered />
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ProfileEditContent {...contentProps} showHeaderClose />
      </div>
    </div>
  );
}
