...
export const EmojiKeyboardTrigger = forwardRef<HTMLButtonElement, EmojiKeyboardTriggerProps>(
  function EmojiKeyboardTrigger({ open, onPress, className = "" }, ref) {
  return (
    <button
      ref={ref}
      type="button"
...
interface EmojiKeyboardPanelProps {
  ...
  /** panel — полоска под composer (mobile); popover — компактная панель над кнопкой (desktop). */
...
export function EmojiKeyboardPanel({
  ...
  const shellClass = isPopover
    ? "flex max-h-[320px] min-h-[240px] w-full flex-col overflow-hidden rounded-2xl border border-border/80 bg-card/95 shadow-xl backdrop-blur-xl"
    : "flex max-h-[min(40dvh,320px)] min-h-[160px] w-full flex-col border-t border-white/10 bg-background/95 shadow-[0_8px_24px_rgba(0,0,0,0.1)] backdrop-blur-xl";

  const pickerHeightClass = isPopover
    ? "h-[280px] min-h-[240px]"
    : "h-[min(36dvh,280px)] min-h-[180px]";

  const loadingHeightClass = isPopover
    ? "h-[240px] min-h-[240px]"
    : "h-[min(36dvh,260px)] min-h-[180px]";

  return (
    <div className={shellClass} role="dialog" aria-label="Выбор эмодзи">
      <div className="flex min-h-0 w-full flex-1 flex-col overflow-hidden px-1 pb-1 pt-0">
        {loadError ? (
          ...
          <div
            className={`flex ${loadingHeightClass} items-center justify-center gap-2 text-sm text-muted-foreground`}
          >
            <Loader2 className="h-5 w-5 shrink-0 animate-spin" aria-hidden />
            Загрузка…
          </div>
        ) : (
          <div className={`emoji-mart-host ${pickerHeightClass} w-full max-w-full`}>
            <Picker
              data={payload.data}
              i18n={payload.i18n}
              locale="ru"
