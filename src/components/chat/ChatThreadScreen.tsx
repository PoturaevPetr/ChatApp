...
} from "@/lib/composerClipboardDismiss";
...
export function ChatThreadScreen({ mode = "standalone" }: { mode?: ChatThreadScreenMode }) {
  ...
  /** Узкий экран: после закрытия клавиатуры WebKit иногда оставляет layout scroll / неверный vv — строка ввода «залипает» сверху. */
  const snapComposerViewportAfterBlur = useCallback(() => {
  ...
  const onAndroidBackButton = useCallback(
    ...
    [router, messageMenu, deleteMessageTarget, mode]
  );
  ...
  useEffect(() => {
    ...
    if (suppressClipboardSuggestionUntilBlurRef.current) {
      setClipboardSuggestion(null);
      return;
    }
    ...
    void readClipboardSuggestion().then((s) => {
      ...
      if (s && isComposerClipboardContentDismissed(s)) {
        setClipboardSuggestion(null);
        return;
      }
  ...
  const clearClipboardSuggestionAfterSend = () => {
    setClipboardSuggestion(null);
    suppressClipboardSuggestionUntilBlurRef.current = true;
    void readClipboardSuggestion().then((s) => {
      if (s) markComposerClipboardContentDismissed(s);
    });
  };
  ...
  if (!peerReady) {
    const loadingPeer = (
      <div className="flex h-full min-h-[200px] flex-col items-center justify-center gap-3 bg-background py-24 text-muted-foreground">
        <Loader2 className="h-10 w-10 animate-spin text-primary" aria-hidden />
        <p className="text-sm">Загрузка профиля…</p>
      </div>
    );
    if (mode === "embedded") return loadingPeer;
    return (
      <AuthGuard requireAuth>
        <Layout>{loadingPeer}</Layout>
      </AuthGuard>
    );
  }
      ...
      .then(async (stream) => {
        ...
        if (mode === "video") {
          ...
          recorder.onstop = () => {
            ...
            if (shouldDiscard || blob.size < MIN_RECORDED_MEDIA_BYTES) {
            ...
            if (recorded.size > MAX_CHAT_ATTACHMENT_BYTES) {
              ...
              setEmojiKeyboardOpen(false);
              void sendMessage(ctx.myId, ctx.peerId, "", { nativeFile: recorded }, ctx.reply ?? undefined).then(() => {
                clearClipboardSuggestionAfterSend();
                scrollThreadToBottomAfterSend();
              });
              setReplyingTo(null);
  ...
  const stopRecording = async (options?: { discard?: boolean }) => {
    ...
    discardCurrentRecordingRef.current = shouldDiscard;
    if (recordingWithNativeEngineRef.current) {
      recordingWithNativeEngineRef.current = false;
      setIsRecording(false);
      try {
  ...
  const threadUi = (
        ...
        >
          <header className="absolute left-0 right-0 top-0 z-30 h-14 w-full shrink-0 border-b border-white/10 bg-background/35 backdrop-blur-xl shadow-[0_10px_30px_-20px_rgba(0,0,0,0.6)] overflow-visible md:relative md:shrink-0">
            ...
            <div className="relative flex h-full min-h-0 items-center gap-2.5 px-4">
              <button
                type="button"
                onClick={leaveChat}
              ...
              <div className="flex min-h-0 min-w-0 flex-1 flex-col justify-center gap-px">
                <div className="flex min-w-0 items-center gap-2">
                  <button
                    type="button"
                    onClick={onHeaderPeerClick}
                    className="truncate rounded text-sm font-semibold leading-none text-foreground hover:underline focus:outline-none focus:ring-2 focus:ring-primary/30"
                ...
                >
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={isSending}
                      className="flex h-11 w-11 shrink-0 self-end items-center justify-center rounded-full border border-destructive/40 bg-background text-destructive shadow-sm transition hover:bg-destructive/10 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-destructive/40 focus:ring-offset-2 focus:ring-offset-background"
                      aria-label="Отменить запись голосового сообщения"
                      title="Отменить"
                    >
            ...
            <div className="pointer-events-none fixed inset-0 z-[10070] flex flex-col items-center justify-center gap-3 p-4">
              ...
              <div className="relative z-10 flex flex-col items-center gap-2.5">
                ...
                <button
                  ...
                  onClick={cancelRecording}
                  disabled={isSending}
                  className="pointer-events-auto flex items-center gap-2 rounded-full border border-white/30 bg-black/30 px-5 py-2.5 text-sm font-semibold text-white shadow-lg transition hover:bg-black/45 disabled:opacity-50 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/70"
                  aria-label="Отменить видеозапись"
                >
